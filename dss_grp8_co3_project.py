# -*- coding: utf-8 -*-
"""(1)Copy of DSS_GRP8_CO3_Project.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IkdQtkUwjqAKFwYoeL7Sd1bIZ3BAlLUA
...
gender_loan_status_counts = df.groupby(['gender', 'loan_status']).size().reset_index(name='count')
display(gender_loan_status_counts)

"""This bar chart illustrates the **Loan Status by Gender** within the dataset. It was generated using Python’s Seaborn and Matplotlib libraries to visualize the loan application outcomes for different genders.

The chart reveals the following distribution:

**For Males:**
*   **Approved (Y):** 339 individuals.
*   **Not Approved (N):** 163 individuals.
*   Total Males: 502 individuals.

**For Females:**
*   **Approved (Y):** 83 individuals.
*   **Not Approved (N):** 29 individuals.
*   Total Females: 112 individuals.

Total Count: 614 individuals.

The data indicates that both male and female applicants have a higher number of approved loans compared to rejected ones. Approximately 67.5% of male applicants received approval, while roughly 74.1% of female applicants were approved. This suggests that female applicants have a slightly higher approval rate in this dataset, although the absolute number of approved male applicants is much higher due to their larger representation in the dataset.
...
#Combined income column
df['total_income'] = df['applicant_income'] + df['coapplicant_income']

#Create Debt to income ratio for better prediction
df['debt_to_income_ratio'] = df['loan_amount'] / (df['total_income'] + 1)

#remove redundancy
df = df.drop(columns='applicant_income')
df = df.drop(columns=['coapplicant_income'])

#Encoded loan status in EDA for heatmap
...
"""Classification Report Analysis This report measures the accuracy, precision, and recall of our model.

**Accuracy (0.86):** The model is correct 86% of the time.

**Precision (Class 0):** When the model predicts a rejection, it is 80% accurate.

**Recall (Class 1):** The model successfully identifies 92% of all eligible loan applicants.
"""